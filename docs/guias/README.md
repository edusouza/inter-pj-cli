...
| [Cobranças Pix](cobrancas-pix.md) | criar uma cobrança imediata (`pix cob`) com o txid que torna segura a repetição, alterar e remover, uma cobrança paga com os seus Pix, conferir uma criação de resultado incerto e as cobranças de um período; criar uma cobrança com vencimento (`pix cobv`) pelas opções ou por um arquivo, com a validade, a multa, os juros e o desconto, e alterá-la; as locations, para um QR Code impresso que serve a uma cobrança depois da outra; e os lotes de cobranças com vencimento, a partir de uma planilha, com a conferência antes do envio, o processamento e as alterações |
