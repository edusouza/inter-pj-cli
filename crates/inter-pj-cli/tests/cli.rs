//! End-to-end tests: the real `inter-pj` binary against a mock Inter API, with
//! configuration, cache and home directories isolated in a temporary folder.
//! Every credential, certificate and account number here is synthetic.
...
const CLIENT_ID: &str = "client-id-e2e-0000-1111";
const CLIENT_SECRET: &str = "segredo-e2e-que-nunca-deve-vazar";
const TOKEN: &str = "token-e2e-que-nunca-deve-vazar";
...
impl TestEnv {
    ...
    async fn mount_token(&self, scope: &str, calls: Option<u64>) {
    ...
    async fn mount_saldo(&self, response: ResponseTemplate, calls: u64) {
