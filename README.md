...
No Pix Automático, o pagador autoriza uma vez, no banco dele, as cobranças de um contrato (uma mensalidade, uma assinatura), que depois são feitas a cada vencimento sem que ele precise pagá-las. A autorização é a **recorrência**: o devedor e o contrato, a periodicidade, o valor (fixo, um mínimo para o limite que o pagador define, ou o de cada cobrança) e se as cobranças não pagas podem ser tentadas de novo. A API é só para CNPJs com pelo menos 6 meses de atividade. Tudo está no guia [Pix Automático](docs/guias/pix-automatico.md): a recorrência (`pix-automatico rec`), criada pelas opções ou por um arquivo JSON com os campos da API (`rec modelo`), com o resumo e a confirmação, a conferência de uma criação de resultado incerto, que esta API não protege com uma chave de idempotência, a listagem, a alteração e o cancelamento; a aprovação pedida ao banco do pagador (`solicitacao`), acompanhada e cancelada; as cobranças recorrentes (`cobr`), com o débito agendado para o dia útil, as tentativas de liquidação, a nova tentativa e o cancelamento; o QR Code da recorrência, pela location (`locrec`), no terminal ou numa imagem; e o sandbox (`pix-automatico sandbox`), em que a CLI faz o papel do pagador e do banco dele, recusado em produção. Cada parte do guia diz os escopos de que os seus comandos precisam: os de `rec`, `solicrec`, `cobr` e `payloadlocationrec`, de leitura e de escrita, e o `pix.write` do sandbox.
