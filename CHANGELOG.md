...
- `inter-pj pagamento lote enviar --arquivo`: lotes de 2 a 150 boletos, contas, tributos e DARFs a partir de um arquivo JSON (os campos da API, com `tipoPagamento`) ou de uma planilha CSV (`,` ou `;`, com ou sem BOM, como o Excel salva). O lote inteiro é conferido antes do envio, e todos os problemas são listados com a linha ou a posição e o campo, sem enviar nada; o resumo mostra os totais por tipo, cada pagamento e avisos (vencimentos passados, valores diferentes dos do código, pagamentos repetidos no arquivo). Mesmos trilhos dos demais pagamentos, com `limite_por_operacao` valendo para cada pagamento do lote (#26).
